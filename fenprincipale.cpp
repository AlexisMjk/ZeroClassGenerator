...
FenPrincipale::FenPrincipale()
{
    ...
    definitionLayout->addRow("Classe &mere :", classeMere);
    ...
    // Définition du layout principal, du titre de la fenétre, etc.
...
{
    // On vérifie que le nom de la classe n'est pas vide, sinon on arréte
    if (nom->text().isEmpty())
    {
        QMessageBox::critical(this, "Erreur", "Veuillez entrer au moins un nom de classe");
        return; // Arrét de la méthode
    }
    ...
    // On crée puis affiche la fenétre qui affichera le code généré, qu'on lui envoie en paramétre
